...
_parser.add_argument('-d', '--database', dest='database',
...
def keyword_valid(cursor, keyword, threshold=10):
    ...
    count = cursor.execute(
        'SELECT COUNT(*) AS count WHERE keyword=?', (keyword,))
...
def get_corpus(cursor, with_rating=False, zodiac_sign=None, keyword=None):
    ...
    base_stmt = 'SELECT interp%s from horoscopes' % (
...
if __name__ == '__main__':
    ...
    with sqlite3.connect(args.database) as conn:
        if not keyword_valid:
            ...
            sys.exit(1)
        corpus = get_corpus(conn.cursor(), zodiac_sign=None,
